...
本层自身不实现网约车车辆周转、车辆占用或空间再分布、派单成功率、动态等待、动态拥堵、Agent 活动取消、方式偏好和方式选择。T2 继续独立负责活动取消和网约车偏好标签，不再持有公交或网约车供给速度参数；T10 使用本层输出的天气后自由流速度和道路容量乘数计算动态道路拥堵，但不会改写 T9 字段。
...
`road_capacity_multiplier` 是同段输出的独立供给状态，不进入 T9 速度公式，也不会在本层反推拥堵、等待或派单结果。下游 T10 仅用它调整正常道路容量，再结合外生聚合交通量计算 v/c 和动态拥堵乘数。
