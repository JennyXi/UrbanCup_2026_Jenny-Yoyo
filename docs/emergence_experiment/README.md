...
独立脚本 `scripts/run_agent_scale_screen.py` 依次测试50、100、200和500 Agents，并在每个规模运行D0–D3、W0–W2、工作日和休息日。默认只运行3 seeds，用于寻找值得进一步做30-seed实验的最小竞争规模。

该筛选有意固定正式P0交通供给：公交班次倍数1.0、网约车供给倍数1.0、道路参考容量14辆/30分钟。Agent增加时不同比例扩大交通供给，因此这是需求压力机制测试，不是上海人口扩张预测。各规模人口仍按相同年龄、就业、数字接入和活动规则生成，但不同规模不是把完全相同的50人简单复制多次。

```bat
python -B -X utf8 -m scripts.run_agent_scale_screen --seed-count 3 --output outputs\agent_scale_screen_3
```

顶层结果包括：

- `scale_system_per_seed.csv`：每个规模、seed、政策、天气和日期的系统结果；
- `scale_group_per_seed.csv`：固定基线接入群体结果；
- `scale_screen_summary.csv`：跨seed均值；
- `d3_spillover_vs_d0.csv`：D3相对D0的受阻老年人收益、60岁以下等待/未满足/通行时间外溢和系统道路变化；
- `candidate_competition_scales.csv`：首次达到网约车需求供给比1、道路量容比1、60岁以下等待增加0.1分钟或未满足增加的测试规模；
- `scale_consistency_checks.csv`：继承数字实验的取消、可用性和身份不变检查。

候选规模只是“本组离散压力测试中首次满足透明规则的规模”，不是人口临界值、城市承载力或校准结论。如果某规则在500 Agents内仍未出现，结果必须保留 `not found within tested scales`，不得继续调参强行制造阈值。正式30-seed实验应选择最小出现稳定竞争的规模及其相邻规模，而不是默认选择结果最显著的500 Agents。

完成3-seed筛选后，若选择100和200 Agents做正式稳健性实验，可用：

```bat
python -B -X utf8 -m scripts.run_agent_scale_screen --agent-counts 100 200 --seed-count 30 --output outputs\agent_scale_100_200_30
```
...
# 200 Agents C0 fleet calibration

Before applying coupon policies at 200 agents, run the independent no-coupon
fleet calibration. It preserves the existing population proportions, behavior,
weather, bus rules, and conserved two-zone ride-hailing mechanism. The three
configured fleet tiers are mechanism scenarios rather than Shanghai estimates:

| Tier | Workday S1/S2 (total) | Rest day S1/S2 (total) |
|---|---|---|
| low | 16/10 (26) | 14/8 (22) |
| middle | 24/15 (39) | 21/12 (33) |
| proportional | 32/20 (52) | 28/16 (44) |

The proportional tier is four times the original 50-agent fleet. The experiment
uses C0 only, disables non-capacity ride-hailing failure, and reports requests,
vehicle failures, waits, fallback, activity completion, total travel time, road
conditions, end-of-day zone distribution, and vehicle-conservation checks.

```text
python -B -X utf8 -m scripts.run_200_agent_c0_fleet_calibration
```

Use the results to identify a tier with observable capacity competition without
system-wide collapse. This is a baseline mechanism choice, not a fitted optimum.

### C3 community/phone assisted booking

For a nondigital elder without family assistance, C3 community/phone coverage
does not change `digital_access`. If that elder receives an elder-reserve coupon,
the channel may place one proxy ride-hailing request. The option enters normal
mode utility and is not a forced ride; the coupon is redeemed only after a
successful conserved-fleet dispatch and otherwise follows the existing expiry
rule. C0-C2 retain the original digital/family availability rules.

### Public coupon coverage threshold scan

The independent 200-agent scan keeps the discount, conserved fleet, behavior,
weather, activities, and dispatch priorities fixed. Only the daily public pool
changes across 0/20/40/60/80 coupons. Any reported travel-time turning point is
a simple mechanism candidate, not a calibrated optimum.

### Elder dispatch priority experiment

R0 uses actual request time followed by the shared base random priority. R1
prioritizes 60+ medical requests and R2 prioritizes all 60+ requests. Priority
applies only when actual request times are equal, so a later request never
preempts an earlier one. Fleet size, mode utility, fares, and C0 no-coupon demand
remain fixed across policies.
