...
- 必要活动去程成功后即视为完成；
- 返程单独模拟；
- 返程失败记录 `stranded_after_activity=true`，但不撤销活动完成，也不增加 mandatory unmet。

## 9. 户外天气暴露

每条实际发生或尝试的 leg 计算透明的简化暴露：
...
W1输出 `heat_exposure_index`，W2输出 `rain_exposure_index`。两者分别统计，不再把降雨风险写入heat exposure。当前指数等于对应天气下的户外暴露分钟数，尚未引入人体热舒适或积水风险模型。
...
| 公交运力倍数 | W0工作日 | W1工作日 | W2工作日 | W0休息日 | W1休息日 | W2休息日 |
|---:|---:|---:|---:|---:|---:|---:|
...
修复后的全部30组“公交运力 × 天气 × 日类型”组合，其最终道路速度都会随网约车供给变化；旧版本中第二轮选择未回写道路状态的问题已修复。
